...
def process_files(predictions_path, test_socratic_path, output_base_dir, index):
    ...
    for prediction, socratic in zip(predictions_data, test_socratic_data):
        ...
        if similarity_score == 1.0:
            ...
            count_0 += 1
        results.append({
            "question_id": prediction.get('id', socratic.get('id')),
    ...
    # plt.show()
    plt.grid(True)
    plot_path = os.path.join(output_base_dir, f"output_{index}_allCOT.png")
...
output_base_dir = "/Users/piyushigoyal/Documents/ETH Zürich/Mrinmaya's lab/Shridhar/GSM8K/test/outputs_allCOT"
