...
def process_files(predictions_path, test_socratic_path, output_base_dir, index):
    ...
    for prediction, socratic in zip(predictions_data, test_socratic_data):
        random_number = random.randint(0, len(prediction['completions']) - 1)
        first_step = extract_first_step(prediction['completions'][random_number])
        if first_step == 0.0:
            continue
        ...
        predictions_final_ans = extract_pred_ans(prediction['completions'][random_number])
        ...
        if first_ans_similarity:
            if final_ans_similarity:
                count_1 += 1
            else:
                count_0 += 1
        results.append({
    ...
    # plt.show()
    plt.grid(True)
    plot_path = os.path.join(output_base_dir, f"output_{index}_randomSingle.png")
...
output_base_dir = "/Users/piyushigoyal/Documents/ETH Zürich/Mrinmaya's lab/Shridhar/GSM8K/test/outputs_randomSingle/"
